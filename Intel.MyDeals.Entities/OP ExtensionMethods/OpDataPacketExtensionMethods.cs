...
namespace Intel.MyDeals.Entities
{
    public static class OpDataPacketExtensionMethods
    {
        ...
        public static void AddSyncActions(this OpDataPacket<OpDataElementType> packet, OpDataPacket<OpDataElementType> fullPacket, List<int> dealIds, AttributeCollection atrbMstr = null)
        {
            ...
            List<int> onChangeFastTrackIds = testPacket.AllDataElements.Where(d => onChangeFastItems.Select(a => a.ATRB_COL_NM).Contains(d.AtrbCd) && d.DcID > 0 && d.HasValueChanged).Select(d => d.DcID).ToList();

            List<int> majorFastTrackNoRedealIds = testPacket.AllDataElements
            ...
            List<int> minorDealIds = testPacket.AllDataElements
                .Where(d => d.AtrbCdIs(AttributeCodes.WF_STG_CD) && (((d.AtrbValue.ToString() == WorkFlowStages.Active || d.AtrbValue.ToString() == WorkFlowStages.Won) && !d.HasValueChanged) || (d.AtrbValue.ToString() == WorkFlowStages.Cancelled && d.HasValueChanged)) && !wonTenderIds.Contains(d.DcID) && (!dealIds.Any() || dealIds.Contains(d.DcID)) && !majorFieldNoRedealIds.Contains(d.DcID) && !majorFastTrackNoRedealIds.Contains(d.DcID))
                .Select(d => d.DcID).ToList();
