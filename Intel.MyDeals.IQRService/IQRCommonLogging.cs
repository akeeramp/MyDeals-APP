...
{
    public static class IQRCommonLogging
    {
        private const string appName = "DQ IQR Sender";
        private const string emailSubjectHeader = "IQR: ";
        ...
        {
            string logFileName = IQRCommonLogging.GetAppSetting("iqrLogFilePath");
            string absolutePathAndFileName = IQRCommonLogging.StartupPath + logFileName;//System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().GetName().CodeBase).Replace("file:\\", "") + logFileName;
        ...
        public static void WriteToLog(string msg)
        {
            ...
            File.AppendAllText(Intel.MyDeals.IQRService.Program._logFile, FormattedMessage);
        }
        ...
        public static void WriteToLogObject(List<string> msg)
        {
            if (msg != null)
            {
                ...
                {
                    File.AppendAllText(Intel.MyDeals.IQRService.Program._logFile, message);
                }
            }

        }

        public static void SetLogger(out IQRCommonLogging.LogWriter fileLogger, out IQRCommonLogging.LogWriter consoleLogger)
        {
            fileLogger = null;
            consoleLogger = new IQRCommonLogging.LogWriter((s, a) =>
            {
        ...
        public static void SendMail(string runMode, VistexDFDataResponseObject responseObj, List<string> lstStatus)
        {
            ...
            string subject = String.Format("MyDeals-VTX [{0}] [Status: {1}] [ENV: {2}]", runMode, responseObj.BatchStatus, IQRCommonLogging.GetAppSetting("ENV"));
            ...
            using (var client = new SmtpClient())
            {
                using (var myMail = new MailMessage())
                {
                    ...
                    myMail.From = new MailAddress("mydeals.integrations@intel.com");
                    ...
                    {
                        IQRCommonLogging.WriteToLog("Exception Received: " + "IQR Send Mail failure -- " + ex.Message + " |Innerexception: " + ex.InnerException + " | Stack Trace: " + ex.StackTrace);
                    }
                }
            }
        ...
        {
            string to_email = "";
            string subject = String.Format("MyDeals-Tender [{0}] [ENV: {1}]", runMode, IQRCommonLogging.GetAppSetting("ENV"));
            ...
            using (var client = new SmtpClient())
            {
                using (var myMail = new MailMessage())
                {
                    ...
                    {
                        IQRCommonLogging.WriteToLog("Exception Received: " + "Tender Send Mail failure -- " + ex.Message + " |Innerexception: " + ex.InnerException + " | Stack Trace: " + ex.StackTrace);
                    }
                }
            }
        }
        ...
        {
            string subject = String.Format("MyDeals-VTX [{0}] [Status: {1}] [ENV: {2}]", runMode, "Exception", IQRCommonLogging.GetAppSetting("ENV"));

            StringBuilder sb = new StringBuilder(body);
            sb.AppendLine("");
            sb.AppendLine("--------------------------------------------------------------------------------");
            sb.AppendLine("IQR.SendDebugMail:");
            ...
            using (var client = new SmtpClient())
            {
                using (var myMail = new MailMessage())
                {
                    ...
                    {
                        IQRCommonLogging.WriteToLog("Exception Received: " + "Send Debug Mail failure -- " + ex.Message + " |Innerexception: " + ex.InnerException + " | Stack Trace: " + ex.StackTrace);
                    }
                }
            }
        }
