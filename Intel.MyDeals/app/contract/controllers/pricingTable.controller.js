...
function PricingTableController($scope, $state, $stateParams, $filter, confirmationModal, dataService, logger, $linq, pricingTableData, productSelectorService, MrktSegMultiSelectService, $uibModal, $timeout, opGridTemplate, confirmationModal, $compile, $location) {
            ...
            if (root.spreadDs !== undefined) {
                ...
                sheet.batch(function () {
                    for (var r = 0; r < data.length; r++) {
                        ...
                        // if Backend deal, PERIOD_PROFILE & AR_SETTLEMENT_LVL should be blank
        ...
        if ($scope.$parent.$parent.curPricingTable.OBJ_SET_TYPE_CD === "KIT" || $scope.$parent.$parent.curPricingTable.OBJ_SET_TYPE_CD === "VOL_TIER" || $scope.$parent.$parent.curPricingTable.OBJ_SET_TYPE_CD === "FLEX") {
            $timeout(function () {
                ...
                if ($scope.pricingTableData.PRC_TBL_ROW != null && $scope.pricingTableData.PRC_TBL_ROW != undefined && $scope.pricingTableData.PRC_TBL_ROW != '') {
                    for (var i = 0; i < $scope.pricingTableData.PRC_TBL_ROW.length; i++) {
                        if ($scope.pricingTableData.PRC_TBL_ROW[i]['NUM_OF_TIERS'] === $scope.pricingTableData.PRC_TBL_ROW[i]['TIER_NBR']) {
                            ...
                            if ($scope.pricingTableData.PRC_TBL_ROW[i]['DC_ID'] === $scope.pricingTableData.PRC_TBL_ROW[r]['DC_ID']) {
                                ...
                                else if (root.spreadDs._data[r]['NUM_OF_TIERS'] > 1 && root.spreadDs._data[r]['TIER_NBR'] != 1) {
                                    root.spreadDs._data[r] = $scope.pricingTableData.PRC_TBL_ROW[r];
                                }
                            } 
                        }
    ...
    function openProdCorrector(currentRow, transformResults, rowData, publishWipDeals, saveOnContinue) {
        var dealType = $scope.$parent.$parent.curPricingTable.OBJ_SET_TYPE_CD;
