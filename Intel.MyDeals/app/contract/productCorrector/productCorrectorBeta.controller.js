...
function ProductCorrectorBetaModalController($filter, $scope, $uibModalInstance, GetProductCorrectorData, ProductSelectorService, productCorrectorService, contractData, RowId, ProductRows, $linq, $timeout, logger, gridConstants, $uibModal, CustSid) {
    ...
    function selectRow(indx) {
        var x;
        vm.curRowIndx = indx;
        vm.curRowId = vm.issueRowKeys[vm.curRowIndx - 1];
        updateRowDCID();
        vm.curRow = !!GetProductCorrectorData.DuplicateProducts[vm.curRowId]
            ? GetProductCorrectorData.DuplicateProducts[vm.curRowId]
            : GetProductCorrectorData.InValidProducts[vm.curRowId];
        vm.curRowProds = [];

        // Manage all products entered for the row's cell
        for (var p = 0; p < GetProductCorrectorData.ProdctTransformResults[vm.curRowId].length; p++) {
            var item = GetProductCorrectorData.ProdctTransformResults[vm.curRowId][p];
            var reason = "Found the Product";
            var status = "Good";
            var cnt = 0;

            if (!!GetProductCorrectorData.DuplicateProducts[vm.curRowId] && !!GetProductCorrectorData.DuplicateProducts[vm.curRowId][item]) {
                reason = "Found multiple matches";
                status = "Issue";
                cnt += GetProductCorrectorData.DuplicateProducts[vm.curRowId][item].length;
            }
            if (GetProductCorrectorData.InValidProducts[vm.curRowId].indexOf(item) >= 0) {
                reason = "Unable to locate the product";
                status = "Issue";
            }

            // Look for valid products with soft warnings
            if (!!GetProductCorrectorData.ValidProducts[vm.curRowId] && !!GetProductCorrectorData.ValidProducts[vm.curRowId][item]) {
        ...
        var curRowLvl = [];
        vm.curRowPrdCnt = 0;
        if (!!GetProductCorrectorData.DuplicateProducts[vm.curRowId]) {
            var dataitem = GetProductCorrectorData.DuplicateProducts[vm.curRowId];
    ...
    var initProducts = function () {
        ...
        for (key in data.ProdctTransformResults) {
            vm.totRows++;
            if (key === vm.curRowIndx) {
    ...
    vm.gridOptionsPotential = {
        ...
        columns: [
            ...
            {
                field: "selected",
                headerTemplate: '<input type=\'checkbox\' ng-click="vm.clickAllProd()" class=\'check with-font\' id="prdChkAll" /><label for="prdChkAll"></label>',
                filterable: false,
                sortable: false,
                width: "50px",
                template: '<input type=\'checkbox\' ng-click="vm.clickProd(#=data.PRD_MBR_SID#)" ng-model="selected" class=\'check with-font\' id="prdChk#=data.PRD_MBR_SID#" /><label for="prdChk#=data.PRD_MBR_SID#"></label>'
            },
            {
                field: "HIER_VAL_NM",
                title: "Product",
    ...
    vm.clickProd = function(dataItem) {
        debugger;
    }
    ...
    function updateRowDCID() {
        $timeout(function () {
            vm.rowDCId = ProductRows.length > 1
                ? ProductRows[vm.curRowIndx - 1].DC_ID
                : ProductRows[0].DC_ID;
        }, 10);
    }
    ...
    function nextRow() {
        if (vm.curRowIndx >= vm.numIssueRows) return;

        selectRow(vm.curRowIndx + 1);
