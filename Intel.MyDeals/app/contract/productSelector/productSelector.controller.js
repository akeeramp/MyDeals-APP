...
function ProductSelectorModalController($filter, $scope, $uibModalInstance, productSelectionLevels, ProductSelectorService, contractData, $timeout) {
    ...
    var getItems = function (item) {
        ...
        if (vm.selectedPathParts.length === 3) {
            var brandName = vm.productSelectionLevels.filter((i) => {
            ...
            if (vm.items.length == 1 && vm.items[0].name == 'NA') {
                if (arrayContainsString(verticalsWithDrillDownLevel4, item.name)) {
                    vm.items = brandName[0].GDM_BRND_NM.split(',').map((i) => {
                        return {
                            name: i,
        ...
        if (vm.selectedPathParts.length === 4) {
            ...
            if (vm.items.length == 1 && vm.items[0].name == 'NA') {
                if (familyName[0].GDM_FMLY_NM != null && familyName[0].GDM_FMLY_NM != "") {
                    vm.items = familyName[0].GDM_FMLY_NM.split(',').map((i) => {
                        return {
                            name: i,
    ...
    function processProducts(data) {
        ...
        vm.searchItems = productCategories.map((i) => {
            return {
                name: i.PRD_CAT_NM,
                level: "VERTICAL",
    ...
    vm.selectsearchItem = function (item) {
        if (item.level == "VERTICAL") {
            ...
            vm.searchItems = markLvl2s.map((i) => {
                return {
                    name: i.MRK_LVL2,
                    vertical: item.name,
        ...
        if (item.level == "MARKLEVEL2") {
            ...
            vm.searchItems = brandNames.map((i) => {
                return {
                    name: i.BRND_NM,
                    level: "Brand",
        ...
        if (item.level == "Brand") {
            ...
            vm.searchItems = familyNames.map((i) => {
                return {
                    name: i.FMLY_NM,
