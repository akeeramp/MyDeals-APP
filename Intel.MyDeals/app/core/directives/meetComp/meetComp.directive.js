                ...
                if (!!$scope.objSid) {
                    ...
                    dataService.get("api/MeetComp/GetMeetCompProductDetails/" + $scope.objSid + "/" + $scope.MC_MODE + "/" + $scope.objTypeId).then(function (response) {
                        ...
                        if (response.data.length > 0) {
                                ...
                                columns: [
                                    {
                                        field: "",
                                        width: "50px",
                                        headerTemplate: "<div class='dealTools' style='margin-left: -15px !important;'><input type='checkbox' class='grid-link-checkbox with-font' id='lnkChk_header' ng-click='selectProdIDS(\"all\", $event)' /> <label for='lnkChk_header' style='margin: 5px 0 0 0;' title='Check to apply changes to all checked rows'></label></div>",
                                        //template: "<div class='dealTools'><input type='checkbox' ng-model='dataItem.IS_SELECTED' class='grid-link-checkbox with-font' id='lnkChk_{{#=RW_NM#}}' ng-click='selectProdIDS(#=RW_NM#, $event, dataItem)' /> <label for='lnkChk_{{#=RW_NM#}}' style='margin: 5px 0 0 0;' title='Check to apply changes to all checked rows'></label></div>"
                                        template: "#if(MEET_COMP_UPD_FLG.toLowerCase() == 'n'){## ##} else {#<div class='dealTools'><input type='checkbox' ng-model='dataItem.IS_SELECTED' class='grid-link-checkbox with-font' id='lnkChk_{{#=RW_NM#}}' ng-click='selectProdIDS(#=RW_NM#, $event, dataItem)' /> <label for='lnkChk_{{#=RW_NM#}}' style='margin: 5px 0 0 0;' title='Check to apply changes to all checked rows'></label></div>#}#"
                                    },
                                    {
                                        field: "PRD_CAT_NM",
                                        title: "Vertical",
                            ...
                            $scope.getMeetCompPopupMessage = function () {
                                var isSKUChanged = false;
                                var isPriceChanged = false;
                                var isIACompBenchChanged = false;
                                for (var i = 0; i < $scope.tempUpdatedList.length; i++) {
                                    var ChangedSKUCount = $linq.Enumerable().From($scope.meetCompUnchangedData)
                                        .Where(function (x) {
                                            return (
                                                x.GRP_PRD_SID == $scope.tempUpdatedList[i].GRP_PRD_SID &&
                                                x.COMP_SKU == $scope.tempUpdatedList[i].COMP_SKU
                                            )
                                        }).ToArray();

                                    var ChangedPRCCount = $linq.Enumerable().From($scope.meetCompUnchangedData)
                                        .Where(function (x) {
                                            return (
                                                x.GRP_PRD_SID == $scope.tempUpdatedList[i].GRP_PRD_SID &&
                                                x.COMP_SKU == $scope.tempUpdatedList[i].COMP_SKU &&
                                                x.COMP_PRC == $scope.tempUpdatedList[i].COMP_PRC
                                            )
                                        }).ToArray();

                                    var changedIACMPBenchCount = $linq.Enumerable().From($scope.meetCompUnchangedData)
                                        .Where(function (x) {
                                            return (
                                                x.GRP_PRD_SID == $scope.tempUpdatedList[i].GRP_PRD_SID &&
                                                x.IA_BNCH == $scope.tempUpdatedList[i].IA_BNCH &&
                                                x.COMP_BNCH == $scope.tempUpdatedList[i].COMP_BNCH
                                            )
                                        }).ToArray();
                                ...
                                if ((isSKUChanged || isPriceChanged) && isIACompBenchChanged) {
                                    return $scope.msgDescDict['SKU_BNCH'];
                                }
                                else if (isIACompBenchChanged) {
                                    return $scope.msgDescDict['IA_COMP_BNCH'];
                                }
                                else if (isSKUChanged) {
                                    return $scope.msgDescDict['COMP_SKU_NEW'];
                                }
                                else if (isPriceChanged) {
                                    return $scope.msgDescDict['COMP_SKU_UPD'];
                                }
                                else {
                                    return null;
                                }
                            ...
                            $scope.saveAndRunMeetComp = function () {
                                ...
                                var unChangedMeetComp = $scope.meetCompUnchangedData;
                                ...
                                if (isValid) {
                                    ...
                                    if ($scope.tempUpdatedList.length > 0) {
                                        var popUpMessage = $scope.getMeetCompPopupMessage();
