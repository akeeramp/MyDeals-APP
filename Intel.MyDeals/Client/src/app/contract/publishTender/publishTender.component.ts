    ...
    publishTenderDeal() {
        ...
        this.publishtenderService.publishTenderDeals(this.contractData[0].DC_ID, this.exlusionList).subscribe((response) => {
            if (response) {
                ...
                window.location.href = "AdvancedSearch#/tenderDashboard?DealType=" + this.contractData[0].PRC_ST[0].PRC_TBL[0].OBJ_SET_TYPE_CD + "&FolioId=" + this.contractData[0].DC_ID + "&search=true";
            }
            else {
                this.isDataLoading=false;
