...
export class ComplexStackingDialogService {
    ...
    public fetchAndShowComplexStakingOverlappingDeals(ovlpObjs): Observable<ComplexStakingStatusInfo> {
        const subject = new Subject<ComplexStakingStatusInfo>();
        this.complexStackingModalsvc.getComplexStackingGroup("COMPLEX_GRP_DATA", ovlpObjs).toPromise().then((ovlpResponse) => {
                ...
                DIALOG_REF.afterClosed().subscribe(async (inputData: DynamicObj[]) => {
                    if (inputData.length > 0) {
                        ...
                        const response = await this.complexStackingModalsvc.updateComplexStackingDealGroup(inputData).toPromise().catch((error) => {
                            ...
                            this.loggerSvc.error('Unable to accept Complex Stacking', error);
                        });
                        if (response) {
                            subject.next({
                                isLoading: false,
                                ...
                                isAccepted: true,
                                GroupingCount: ovlpResponse.GroupItems.length
                            });
