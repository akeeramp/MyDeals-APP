...
import { ProductDetailsReportService } from './productDetailsReport.service';
...
@Component({
    ...
    templateUrl: 'Client/src/app/reporting/productDetailsReport/productDetailsReport.component.html',
    styleUrls: ['Client/src/app/reporting/productDetailsReport/productDetailsReport.component.css'],
    ...
    responseLnav(e: any, sKey?) {
        ...
        if (this.flagBool == "reportingdashboard") {
            this.list_breadcrumbs = [
                ...
                { text: "Product Details Report", url: "javascript:void(0)" }
            ];
        }
    }
    ...
    CheckURLAccess() {
        this.constantsSvc.getConstantsByName("NEW_ACCOUNT_REVIEWER_Worldwide").pipe(takeUntil(this.destroy$))
            .subscribe((response) => {
                ...
                if (this.checkUsrEmail || (<any>window).isDeveloper) {
                    this.isEligibleToViewPrdDtl = true;
                    this.router.navigate(['/productdetailsreport']);
                }
                else {
