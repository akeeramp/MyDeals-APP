...
import {logger} from '../logger/logger';
    ...
    function ReportingDashboardController($scope, reportingService, $linq) {
        ...
        var loadReportDashboard = function () {
            reportingService.getReportData()
                .then(function (response) {
                    ...
                    if ((<any>window).usrRole != 'RA' && !((<any>window).isSuper || (<any>window).isDeveloper)) {
                    ...
                    if (vm.masterData["ReportLogDeatils"].length > 0) {
                        ...
                        for (var countBar = 0; countBar < vm.masterData["ReportDashboardData"].length; countBar++) {
                            ...
                            var dataColor = { category: vm.masterData["ReportDashboardData"][countBar].RPT_NM, value: (vm.masterData["ReportDashboardData"][countBar].TOTAL_TIME * 100 / vm.masterData["ReportDashboardData"][countBar].TOTAL_REPORT_TIME).toFixed(2), color: CLR_CD };
                    ...
                    logger.error("Unable to get Report Data.", response);
                });
        }
