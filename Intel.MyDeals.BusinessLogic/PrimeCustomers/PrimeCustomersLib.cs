...
namespace Intel.MyDeals.BusinessLogic
{
    public class PrimeCustomersLib : IPrimeCustomersLib
    {
        ...
        public bool UpdateUnPrimeDeals(int dealId , UnPrimeAtrbs endCustData,bool isUnificationMailRequired=true)
        {
        ...
        public string UnPrimeDealsLogs(string dealIdData, string endCustData, bool isRetry = false)
        {
            //Note: if multiple deals are selected in UI or Deal tools is checked, Only in that case dealIdData contains multiple deal id's. even dealIdList has multiple deals, only one API request is sent to UCD
            List<int> dealIdList = dealIdData.Split(',').Select(int.Parse).ToList();
            int dealId = dealIdList[0];
                ...
                while (result.Any())
                {
                    ...
                    foreach (UnPrimedDealLogs Customer in batch)
                    {
                            ...
                            (Customer.PRIMED_CUST_CNTRY != null && Customer.PRIMED_CUST_CNTRY != "") && dealId != 0)
                        {
                            ...
                            foreach (int dealID in dealIdList)
                            {
        ...
        public void saveDealEndCustomerAtrbs(int dealId, string endCustObjdata, bool isUnificationMailRequired, int creEmpWwid)
        {
            ...
            UpdateUnPrimeDeals(dealId, data, isUnificationMailRequired);
        }
                            ...
                            if (responseforSurvivorAccIDReq != null)
                            {
                                ...
                                else
                                {
                                    _primeCustomersDataLib.SaveUcdRequestData(res.AccountName, res.primaryAddress.CountryName,
                                         0, null, UCDDupResponse, res.AccountId, "API_processing_Error");
                                }
                            }
                            else
                            {
                                _primeCustomersDataLib.SaveUcdRequestData(res.AccountName, res.primaryAddress.CountryName,
                                     0, null, null, res.AccountId, "API_processing_Error");
                            }
                        }
        ...
        public string ResubmissionDeals(string dealId, string endCustomerData)
        {
            ...
            try
            {
                //Checking whether the deals are IQR deals or not as Retrigger UCD request is Enabled only for My Deals originated deals.
                bool isNotIqrDeal = _primeCustomersDataLib.CheckForIQRDeals(dealId);
                ...
                {
                    OpLogPerf.Log("Retrigger UCD request is Enabled only for My Deals originated deals and not IQR Deals");
                    success = "IQR Deals";
                }
            }
            catch (Exception ex)
