...
namespace Intel.MyDeals.BusinessLogic
{
    public class ProductsLib : IProductsLib
    {
        ...
        public ProductLookup TranslateProductsTenders(ContractToken contractToken, List<ProductEntryAttribute> prodNames, int PRD_MBR_SID, int CUST_MBR_SID, string DEAL_TYPE, bool IS_TENDER)
        {
            PRD_MBR_SID = 78227;
            DateTime start = DateTime.Now;
            Stopwatch stopwatch = new Stopwatch();
            ...
            var productMatchResults2 = productMatchResults.Where(p => p.PRD_MBR_SID == PRD_MBR_SID).ToList();

            productLookup.ProdctTransformResults = concurrentProdctTransformResults.ToDictionary(kvp => kvp.Key,
                                                          kvp => kvp.Value);
            // Get duplicate and Valid Products
            ExtractValidandDuplicateProducts(productLookup, productMatchResults2);
