...
//        private readonly IDataCollectorLib _dataCollectorLib;
        ...
        public MyDealsData SavePackets(OpDataCollectorFlattenedDictList data, int custId)
        {
            ...
            MyDealsData myDealsData = _dataCollectorDataLib
                .GetByIDs(opTypeGrp, ids, opDataElementTypes) // Go to DB and get what is there - in our format
                .Merge(data) // takes passed data and DB and makes one set
                .FillInHolesFromTemplate(); // takes template and adds any needed missing DEs for rules
            ...
            foreach (OpDataElementType opDataElementType in Enum.GetValues(typeof(OpDataElementType)))
            {
                ...
                if (opDataElementType == OpDataElementType.Deals) // Deal DC setup
                {
                    SaveDealsbyDictionary(data[opDataElementType], myDealsData, myBatchId);
                }
                else // Non-Deal DC setup - Likely these are merged in MyDeals since it is all the same name
                {
        ...
        private MyDealsData PerformTasks(OpActionType? actionToRun, MyDealsData myDealsData, int custId)
        {
            ...
            switch (actionToRun)
            {
                case OpActionType.Save:
                    ...
                    saveResponseSet = new DataCollectorDataLib().SaveDeals(myDealsData, custId);
