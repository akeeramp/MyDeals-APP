...
namespace Intel.MyDeals.BusinessRules
{
    ...
    public static partial class MyDcActions
    {
        ...
        public static void ExecuteMerges(params object[] args)
        {
            ...
            {
                // Normal Consumption set to deal start/end by default if empty
            ...
            else if (payoutBasedOn.Equals("Consumption", StringComparison.InvariantCultureIgnoreCase) && dcRebateType.Equals("TENDER", StringComparison.InvariantCultureIgnoreCase))
            {
                // Tenders get 2 years prior by default if value is empty
                if (string.IsNullOrEmpty(billStartDate?.AtrbValue.ToString()))
                {
                    item[AttributeCodes.REBATE_BILLING_START] = DateTime.Parse(dcSt).AddYears(-2).ToString("MM/dd/yyyy");
                    item[AttributeCodes.REBATE_BILLING_END] = DateTime.Parse(dcEn).ToString("MM/dd/yyyy");
                }
            }
            else // Billings Deals
        ...
        public static void CheckBillingDates(params object[] args)
        {
            ...
            if (deStart.HasValueChanged && !deBllgStart.HasValueChanged)
            {
                ...
                {
                    if (rebateType.Equals("TENDER", StringComparison.InvariantCultureIgnoreCase))
                    {
                        deBllgStart.SetAtrbValue(dtSt.AddYears(-2).ToString("MM/dd/yyyy")); // Set billings date to 2 years prior
                    }
                    else
                    {
                        deBllgStart.SetAtrbValue(dtSt.ToString("MM/dd/yyyy")); // Set to Start Date
                    }
                }
            }
        ...
        public static void CheckTenderOverlaps(params object[] args)
        {
            ...
            if (SfConditionsCheck && overlapsCheckDeals.Count > 0)
            {
                ...
                deEndCustomer.AddMessage("This deal overlaps with another deal by having the same Customer, End Customer, Project, Product, Overlapping Dates, Customer Platform, Customer Segment, Customer Reported Sales Geo and Consumption Country/Region. Please make changes to this deal so that it no longer overlaps this existing deal(s)  [" + overlaps + "].");
            }
        }
        ...
        public static void MajorChangeCheck(params object[] args)
        {
            ...
            if (wipStage == WorkFlowStages.Active || wipStage == WorkFlowStages.Won) // WIP Object, Set re-deal date only if this came from active since it will drive the tracker effective from/to date calc.
            {
                ...
                r.Dc.SetAtrb(AttributeCodes.IS_PCT_MCT_FAILURE_SKIPPED, 0);
            ...
            if ((!primedCheck || rplCheck || rplStatusCodeCheck || isNoRplStatusAvailable) && deEndCust.AtrbValue.ToString() != "" && !salesForceCheck && !isPnr && isCancelled != "1") // If not cancelled, not primed and End customer has a value
            {
                ...
                else if (!isTender)
                {
                    if (!(programPayment.Contains("Frontend") && dealStage == WorkFlowStages.Active))
                    {
                        ...
                        {
                            deEndCust.AddMessage("Intel cannot approve a transaction with the specified End Customer.  The activities conducted within My Deals are subject to U.S. and other applicable export control and sanctions laws.");
                        }
                        else if (rplStatusCodeCheck)
                        {
        ...
        public static void ValidatePrimeCountry(params object[] args)
        {
            ...
            foreach(var ctryVal in endCustCountryVal) {
                ...
                if(custCountry != null) {
                    ...
                    {
                        deEndCust.AddMessage("Intel is currently unable to approve deals with the selected End Customer Country/Region. Please verify the agreement.");
                    }                    
                }
            }
        }
        ...
        public static void NreMdfSupportedGroupTypes(params object[] args)
        {
            MyOpRuleCore r = new MyOpRuleCore(args);
            if (!r.IsValid) return;

            List<string> targetTypes = new List<string> { "NRE", "MDF" };
            List<string> newtargetTypes = new List<string> { "CO-"};
            string myRebateType = r.Dc.GetDataElementValue(AttributeCodes.REBATE_TYPE);
            var dealType = r.Dc.GetDataElementValue(AttributeCodes.OBJ_SET_TYPE_CD);
