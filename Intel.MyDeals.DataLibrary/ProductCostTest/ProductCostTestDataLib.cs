...
namespace Intel.MyDeals.DataLibrary
{
    public class ProductCostTestDataLib : IProductCostTestDataLib
    {
        ...
        /// </summary>
        /// <returns></returns>
        public List<LegalExceptionExport> DownloadLegalException(string data, bool excludeCurrVer,bool dealList)
        {
            var ret = new List<LegalExceptionExport>();
            try
            {
                in_t_lgl_excpt_dwnld dt1 = new in_t_lgl_excpt_dwnld();                
                List<int> excpt_SID = new List<int>(Array.ConvertAll(data.Split(','), int.Parse));                    
                dt1.AddRows(excpt_SID);
                

                Procs.dbo.PR_MYDL_GET_PCT_LGL_EXCPT_DWNLD cmd = new Procs.dbo.PR_MYDL_GET_PCT_LGL_EXCPT_DWNLD
                {

                    in_pct_excpt_lst = dt1,
                    in_incld_all_excpt_ver = excludeCurrVer,
                    in_incld_excpt_deal = dealList
                };
                using (var rdr = DataAccess.ExecuteReader(cmd))
                {
                    int IDX_ACTV_IND = DB.GetReaderOrdinal(rdr, "ACTV_IND");
